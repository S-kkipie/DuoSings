...
function Monito() {
  ...
  const [isPopup, setIsPopup] = useState(false); // Estado para definir si es popup
  const [input, setInput] = useState<string>("");
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          setIsPopup(!entry.isIntersecting); // Cambia a popup cuando no es visible
        });
      },
      { threshold: 0.1 }, // Detectar visibilidad al 10%
    );
    ...
    return () => {
      if (divRef.current) observer.unobserve(divRef.current); // Limpieza
    };
  }, []);
  const {
    currentWord,
  ...
  return (
    <>
      <div
        className={
          "mt-12 flex h-[500px] w-full max-w-[80rem]" + !isPopup ? "" : "hidden"
        }
        ref={divRef}
      ></div>
      <div
        className={
          `relative z-20 mt-12 flex h-[500px] w-full max-w-[80rem] flex-col items-center justify-center overflow-hidden rounded-lg border bg-background transition md:shadow-xl ` +
          (isPopup ? "!fixed left-2 top-12 h-auto !w-[500px]" : "")
        }
      >
        <button
        onClick={() => setIsPopup(false)}
          className={
            !isPopup
              ? "hidden"
              : "z-20 absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground"
          }
        >
        ...
        <div className={!isPopup ? "hidden" : "my-3 flex h-8 w-full p-1"}>
          <Input
          ...
          <Button
            onClick={() => {
              socket!.emit("E-REQUEST-ANIMATION", input);
            }}
          >
            <Send></Send>
          </Button>
  ...
  return (
    <div className="mx-auto flex-1 overflow-hidden">
      ...
      >
        ...
        <Link
          href="/learn"
          className="animate-fade-in inline-flex h-9 translate-y-[-1rem] items-center justify-center gap-1 whitespace-nowrap rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white shadow transition-colors ease-in-out [--animation-delay:600ms] hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 dark:text-black"
        >
          <span>Empieza tu aprendizaje! </span>
          <svg
            width="15"
            height="15"
            viewBox="0 0 15 15"
                            ...
                            <div className="flex space-x-4 md:space-x-2 md:space-x-reverse">
                              ...
                              >
                                <AccordionTrigger className="text-xl font-semibold text-primary">
                                  ...
                                  <span className="mx-5 text-sm text-secondary">
                                    {module.duration}
                                  </span>
                                </AccordionTrigger>
                                <AccordionContent>
